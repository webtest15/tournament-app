...
function buildBracket(participants) {
  ...
  // Build slots: spread BYEs evenly so no two BYEs face each other.
  // Strategy: place one BYE at the end of each "group" of ceil(size/numByes) slots.
  // Simple approach: interleave BYEs as evenly as possible across match pairs.
  // We have size/2 matches. byeCount BYEs. Place at most 1 BYE per match.
  const numMatches = size / 2;
  // Which match indices get a BYE? Spread evenly.
  const byeMatchIndices = new Set();
  if (numByes > 0) {
    for (let k = 0; k < numByes; k++) {
      // Evenly distribute: pick index = round(k * numMatches / numByes)
      byeMatchIndices.add(Math.round(k * numMatches / numByes));
    }
  }

  // Build slots array: for bye matches put (player, null), for others put (player, player)
  const slots = []; // array of [p1, p2] pairs
  let pi = 0; // participant index
  for (let i = 0; i < numMatches; i++) {
    const isBye = byeMatchIndices.has(i);
  ...
  const rounds = [];
  // Round 0: build from slots
  const firstRound = slots.map(([p1, p2]) => {
    const isBye = !p2;
    return {
      id: uid(),
  ...
  let roundSize = numMatches;
  while (roundSize > 1) {
    roundSize = roundSize / 2;
    const r = Array.from({ length: roundSize }, () => ({
...
function propagate(bracket) {
  ...
  // Resolve a match: returns the advancing team, or null if undecided.
  // Only advances if winner is explicitly set (human click or round-0 BYE auto-set).
  // Does NOT auto-advance based on "only one team present" — that is handled by syncWinner.
  const resolve = (m) => {
  ...
  // Sync winner only for BYE matches (one team, no opponent).
  // For matches that already have both teams, preserve existing winner.
  const syncWinner = (m, allowAutoBye) => {
    if (allowAutoBye && m.team1 && !m.team2) { m.winner = 1; m.bye = true; return; }
    if (allowAutoBye && m.team2 && !m.team1) { m.winner = 2; m.bye = true; return; }
    if (!m.team1 && !m.team2) { m.winner = null; return; }
    // Both present (or auto-bye not allowed): keep existing winner, clear if stale
    if (m.winner === 1 && !m.team1) m.winner = null;
    if (m.winner === 2 && !m.team2) m.winner = null;
  };

  // Round 0: auto-resolve BYEs (no opponent at all)
  for (const m of b.rounds[0]) syncWinner(m, true);

  // Propagate round by round
  for (let r = 0; r < b.rounds.length - 1; r++) {
    const cur  = b.rounds[r];
    const next = b.rounds[r + 1];

    for (let i = 0; i < next.length; i++) {
      const a1 = resolve(cur[i * 2]);
      const a2 = resolve(cur[i * 2 + 1]);

      next[i].team1 = a1?.t || null; next[i].flag1 = a1?.f || "";
      next[i].team2 = a2?.t || null; next[i].flag2 = a2?.f || "";

      // Round 1+: BYE winners have already advanced one round into this slot.
      // If only one team is here it means the other side has no winner yet —
      // do NOT auto-win; wait for the other side to produce a winner.
...
function PinModal({ onSuccess, onCancel }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999 }}>
      <div style={{ background:"#fff",border:`1px solid ${C.border}`,borderRadius:16,padding:"32px 36px",minWidth:280,textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{fontSize:28,marginBottom:8}}>🔐</div>
        <div style={{fontFamily:"'Noto Sans JP'",fontSize:17,fontWeight:700,color:C.text,marginBottom:4}}>管理者ログイン</div>
        ...
        <input type="password" maxLength={4} value={pin}
          ...
          style={{ ...baseInput, textAlign:"center", fontSize:22, letterSpacing:8,
            border:`1px solid ${err?RED:C.border}`, animation:err?"shake 0.3s ease":"none" }}/>
...
function MatchCard({ match, onSetWinner, isAdmin }) {
  // ✅ FIX: BYE match with a real team1 should still be clickable (to confirm winner)
  // Only block clicking if BOTH teams are missing, or if it's genuinely unresolvable
  const hasRealMatch = match.team1 && match.team2;
  const isByeAuto   = match.bye && match.team1 && !match.team2; // auto-won, no opponent
  // Clickable: admin + has team1 + (real match OR not a bye-auto)
...
function BracketView({ bracket, onSetWinner, isAdmin }) {
  ...
  const getLbl = (r) => {
    ...
    return fromEnd===0?"決勝":fromEnd===1?"準決勝":fromEnd===2?"準々決勝":`第${r+1}回戦`;
  };
      ...
      <div style={{ position:"relative", width:totalW, height:totalH, minWidth:totalW }}>
        ...
        {bracket.champion && (
          <div style={{
            ...
            boxShadow:"0 8px 24px rgba(232,57,58,0.18)",
...
// ─── Member Screen ────────────────────────────────────────────────────────────
function MemberScreen({ members, setMembers, isAdmin, onGenerate }) {
    ...
    <div style={{maxWidth:520,margin:"0 auto"}}>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:24,marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <span style={{fontSize:13,fontWeight:700,color:C.text,letterSpacing:1}}>参加メンバー</span>
          <span style={{background:"#fef2f2",border:"1px solid #fca5a5",
            borderRadius:20,padding:"2px 10px",fontSize:11,color:RED,fontWeight:600}}>{members.length}人</span>
        </div>
        {members.length===0 ? (
      ...
      {isAdmin && members.length>=2 && (
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:20,textAlign:"center",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
          ...
          <button onClick={onGenerate} style={{...baseBtn(true),padding:"12px 32px",fontSize:14,letterSpacing:2,fontFamily:"'Bebas Neue'"}}>
            トーナメント生成 →
          </button>
        </div>
      )}
    ...
    <div style={{maxWidth:520,margin:"0 auto"}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:24,marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{fontSize:13,fontWeight:700,color:C.text,letterSpacing:1}}>出場順序を確認</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {participants.map((m,i)=>(
            <div key={m.id} draggable
              ...
              style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",
                background:"#f8fafc",
                border:`1.5px solid ${dragIdx===i?RED:C.border}`,
          ...
          {Array.from({length:size-participants.length}).map((_,i)=>(
            <div key={`bye${i}`} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",
              ...
              <span style={{fontSize:13,color:C.muted,fontStyle:"italic",fontFamily:"'Noto Sans JP',sans-serif"}}>BYE（不戦勝）</span>
            </div>
          ))}
        </div>
      </div>
...
function TournamentApp({ roomId, onLeave }) {
  ...
  const lastMembersRef = useRef(null);
  ...
  const tabStyle=(active)=>({
    padding:"8px 18px",borderRadius:7,fontSize:12,fontFamily:"'Noto Sans JP',sans-serif",cursor:"pointer",
  ...
  return (
    <div style={{
      minHeight:"100vh",
      background:C.bg,
      color:C.text,fontFamily:"'Noto Sans JP',sans-serif",
      ...
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        @media(max-width:480px){
          .tab-nav{flex-wrap:wrap;gap:6px!important;}
          .tab-nav button{font-size:11px!important;padding:6px 10px!important;}
        }
      `}</style>
        ...
        <div style={{marginTop:10}}>
          {isAdmin?(
            <div style={{display:"inline-flex",alignItems:"center",gap:8,
              background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:20,padding:"4px 14px"}}>
              <span style={{fontSize:11,color:RED}}>🔓 管理者モード</span>
              <button onClick={()=>setIsAdmin(false)}
                style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:11,padding:0}}>ログアウト</button>
            </div>
          ):(
            <button onClick={()=>setShowPin(true)} style={{...baseBtn(),fontSize:11,padding:"6px 16px",borderRadius:20}}>🔐 管理者ログイン</button>
          )}
        </div>
        <div style={{marginTop:6,fontSize:10,color:syncing?RED:C.muted,transition:"color 0.3s",letterSpacing:"0.5px"}}>
          {syncing?"⟳ 同期中...":lastSync?`🔄 ${lastSync.toLocaleTimeString("ja-JP")}`:"　"}
        </div>
      </div>
      ...
      {/* Screens */}
      {screen==="members"&&<MemberScreen members={members} setMembers={setMembers} isAdmin={isAdmin} onGenerate={goToSetup}/>}
      {screen==="setup"&&isAdmin&&<SetupScreen members={members} onStart={startTournament} onBack={()=>setScreen("members")}/>}
      {screen==="bracket"&&bracket&&(
        <div>
          <div style={{
            display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,
            maxWidth:900,margin:"0 auto 16px",padding:"10px 16px",
            background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",
          }}>
